// Official BLAKE3 test vectors, all 35 cases, transcribed from
// https://github.com/BLAKE3-team/BLAKE3/blob/master/test_vectors/test_vectors.json
// Input is a repeating sequence of 251 bytes: 0, 1, 2, ..., 250, 0, 1, ...
// Only the first 32 bytes of each extended output are compared, which is the
// default-length hash.
...
describe('blake3', () => {
  it.each(VECTORS)('matches the official vector for input_len %i', (len, expected) => {
    expect(blake3Hex(patternInput(len))).toBe(expected);
  });
});
